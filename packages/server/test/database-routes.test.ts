...
describe('database routes', () => {
  ...
  it('GET /api/db/:name shows owners and the caller role; 403 for non-members', async () => {
    ...
    expect(
      (await app.inject({ method: 'GET', url: '/api/db/kb', headers: { cookie: bob } })).statusCode,
    ).toBe(403);
  });
  ...
  it('owner grants/revokes roles; only owners may grant', async () => {
    ...
    expect(
      (await app.inject({ method: 'GET', url: '/api/db/kb', headers: { cookie: bob } })).json(),
    ).toMatchObject({
      role: 'editor',
    });
  ...
  it('DELETE removes the database (owner only); non-owner editor gets 403', async () => {
    ...
    expect(
      (await app.inject({ method: 'DELETE', url: '/api/db/kb', headers: { cookie: bob } }))
        .statusCode,
    ).toBe(403);
    expect(
      (await app.inject({ method: 'DELETE', url: '/api/db/kb', headers: { cookie: ada } }))
        .statusCode,
    ).toBe(204);
    expect(
      (await app.inject({ method: 'GET', url: '/api/db/kb', headers: { cookie: ada } })).statusCode,
    ).toBe(404);
  });
});
